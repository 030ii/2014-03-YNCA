...
io.on('connection', function (socket) {
	...
	socket.on('inputPoint', function (point) {
		...
		io.sockets.in(socket.room.roomName).emit('setCounterInfo', game.getPlayerInfo(socket.player), socket.player.num);
	...
	var gc = {
		...
		finishRoundAndSendInfo : function (winner, round) {
			if(winner) {
				...
				io.sockets.in(socket.room.roomName).emit('reduceVideoBlur', winner.otherPlayer.num);
			} else {
